...
export function EditCourseForm({ open, onOpenChange, course, onSuccess }: EditCourseFormProps) {
  ...
  React.useEffect(() => {
    if (course && open) {
      setFormData({
        ...
        status: (course.status || "") as "" | "active" | "inactive" | "completed" | "upcoming" | "ongoing" | "cancelled",
      })
    }
  }, [course, open])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const courseData: Partial<Omit<Course, "id" | "created_at" | "updated_at">> = {
