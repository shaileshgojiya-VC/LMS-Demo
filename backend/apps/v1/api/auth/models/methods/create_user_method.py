"""
Database methods for user creation operations.
"""

from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
from typing import Optional

from apps.v1.api.auth.models.model import Users


async def get_user_by_email(db: AsyncSession, email: str) -> Optional[Users]:
    """
    Get user by email address.

    Args:
        db: Async database session
        email: User email address

    Returns:
        Users object if found, None otherwise
    """
    stmt = select(Users).where(Users.email == email).limit(1)
    result = await db.execute(stmt)
    return result.scalar_one_or_none()


async def create_user(db: AsyncSession, user_data: dict) -> Users:
    """
    Create a new user in the database.

    Args:
        db: Async database session
        user_data: Dictionary containing user data (email, password, full_name)

    Returns:
        Created Users object
    """
    new_user = Users(**user_data)
    db.add(new_user)
    await db.commit()
    await db.refresh(new_user)
    return new_user

